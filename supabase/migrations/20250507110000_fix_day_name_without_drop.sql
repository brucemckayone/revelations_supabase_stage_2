-- Fix for day name formatting in service availability functions
-- The issue: TO_CHAR(date, 'day') returns a string with trailing spaces which causes the join with the availability table to fail

-- Update the get_provider_availability function with TRIM to fix day name comparison without dropping it
