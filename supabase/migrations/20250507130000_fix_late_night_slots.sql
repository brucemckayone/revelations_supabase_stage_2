-- Further fix for time slots appearing outside of availability window
-- The issue: Late night slots (23:00, 23:30) are still appearing when they shouldn't
...
BEGIN
    ...
    day_availability AS (
        SELECT
            ...
            av.start_time,
            av.end_time,
            COALESCE(av.is_active, false) AS is_active -- Explicitly handle NULL as false
        FROM
            date_series ds
        LEFT JOIN
    ...
    available_slots AS (
        SELECT
            ...
            CASE
                -- First check: if day is not active, all slots are unavailable
                WHEN NOT da.is_active THEN false
                ...
                WHEN ex.exception_date IS NOT NULL AND ex.is_available AND
                     ...
                     (
                        ...
                        (ex.end_time < ex.start_time AND ts.slot_time >= ex.start_time)
                     ) THEN true
                     
                -- Handle regular availability - normal case (start_time < end_time)
                WHEN da.end_time > da.start_time AND 
                     ts.slot_time >= da.start_time AND 
                     (ts.slot_time + '30 minutes'::interval) <= da.end_time THEN true
                     
                -- Handle regular availability - crossing midnight case
                WHEN da.end_time < da.start_time AND 
                     (
                        (ts.slot_time >= da.start_time) OR
                        (ts.slot_time < da.end_time)
                     ) THEN true
...
COMMENT ON FUNCTION get_provider_availability IS 'Returns detailed availability slots for a provider with special handling for time slots crossing midnight and inactive days'; 