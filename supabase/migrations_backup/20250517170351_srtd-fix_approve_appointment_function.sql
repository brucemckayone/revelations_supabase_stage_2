...
BEGIN
  ...
  -- Try to find or create a chat room between the provider and client
  SELECT id INTO v_chat_room_id
  FROM chat_rooms
  WHERE 
    type = 'private' AND
    ((SELECT COUNT(*) FROM chat_participants WHERE chat_room_id = chat_rooms.id AND user_id IN (v_user_id, v_owner_id)) = 2);
  ...
  IF v_chat_room_id IS NULL THEN
    ...
    VALUES (v_chat_room_id, v_user_id), (v_chat_room_id, v_owner_id);
  END IF;
