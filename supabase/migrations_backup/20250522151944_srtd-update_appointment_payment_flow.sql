-- Generated with srtd from template: supabase/migrations-templates/update_appointment_payment_flow.sql
-- You very likely **DO NOT** want to manually edit this generated file.

BEGIN;

-- Update appointment payment flow with checkout URL generation
...
DROP FUNCTION IF EXISTS public.reschedule_appointment_request;
DROP FUNCTION IF EXISTS public.respond_to_appointment_request;
DROP FUNCTION IF EXISTS public.suggest_appointment_dates;

-- Function to approve appointment requests and generate checkout URL
...
BEGIN
  ...
  -- Generate checkout URL with encoded data and using a proper path structure
  -- This allows secure data passing without requiring auth checks on the ID
  v_checkout_url = p_checkout_base_url || '/secure' || '?data=' || v_encoded_data;


  -- Update appointment status
  UPDATE appointment_purchases
  SET payment_link = v_checkout_url,
      updated_at = NOW()
  WHERE id = p_appointment_id;
  
  -- Find a private chat room that contains EXACTLY these two users and no others
  ...
  -- If no chat room exists, create one with descriptive name
  ...
  ELSE
    -- Even if room exists, ensure both users are participants and update room name to be more descriptive
    UPDATE chat_rooms
  ...
  -- Compose message to include the checkout URL, regardless of p_message
  v_message_text := 
    CASE 
  ...
  -- Send message in chat with payment link (checkout url is always included in the message)
  BEGIN
  ...
  -- Build result with payment metadata and checkout URL
...
COMMENT ON FUNCTION public.approve_appointment_request(UUID, NUMERIC, TEXT, TEXT) IS 'Approves an appointment request, generates payment checkout URL with encoded data, and sends notification with payment link';


-- Update process_appointment_payment_confirmation to generate meeting URL for online/hybrid services
...
BEGIN
  ...
  -- Get appointment details including service information
  SELECT 
    ap.id, 
    ap.appointment_date, 
    ap.service_id,
  ...
  -- Generate meeting URL for online or hybrid services
  IF v_method = 'video' OR (v_service_type IN ('online', 'hybrid')) THEN
    -- Generate a unique meeting URL using UUID v4
    v_meeting_url := 'https://local.revelations.com/meeting/private/' || gen_random_uuid();
    
    -- Update appointment with meeting URL and remove meeting_id
    UPDATE appointment_purchases
  ...
  -- Prepare message text based on appointment type
  IF v_meeting_url IS NOT NULL THEN
  ...
  BEGIN
    -- Message to client with meeting URL if applicable
    INSERT INTO chat_messages (
      chat_room_id,
      sender_id,
      message,
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
      ...
      -- Check owner notification preferences for appointments
      ...
      -- Notify provider (owner) based on preferences
      ...
      INSERT INTO notifications (
        ...
        'Appointment Payment Received',
          'Payment completed for appointment on ' || to_char(v_appointment_date, 'FMDay, FMDD Month YYYY at HH12:MI AM') || 
          CASE WHEN v_meeting_url IS NOT NULL THEN E'\n\nMeeting URL: ' || v_meeting_url ELSE '' END,
        'appointment',
        '/appointments/' || v_appointment_id,
        v_appointment_id,
        'appointment',
  ...
  -- Trigger email notification (for system integration)
...
COMMENT ON FUNCTION public.process_appointment_payment_confirmation(UUID, TEXT) IS 'Processes payment confirmation, generates meeting URL for online/hybrid appointments, removes meeting_id field, and sends notifications based on user preferences';

-- Create or replace the reschedule_appointment_request function to use chat_messages instead of messages
...
BEGIN
  ...
  IF v_chat_room_id IS NULL THEN
    ...
    ON CONFLICT (chat_room_id, user_id) DO NOTHING;
  ELSE
    -- Update chat room name to reflect the new appointment date
    UPDATE chat_rooms
    SET name = v_chat_room_name
    WHERE id = v_chat_room_id;
  END IF;
  
  -- If it's an online/hybrid appointment and needs a meeting URL, generate one
  IF v_meeting_url IS NULL AND (v_method = 'video' OR v_service_type IN ('online', 'hybrid')) THEN
    ...
    BEGIN
      ...
      IF v_user_preference.in_app THEN
        INSERT INTO notifications (
          ...
          v_user_id,
          v_service_name || ' Appointment Rescheduled',
          v_message_text,
          'appointment',
          COALESCE(v_payment_url, v_confirm_url), -- Use payment URL if available, otherwise confirmation URL
          p_appointment_id,
          'appointment',
          v_notification_metadata
        );
      ...
      IF v_user_preference.sms THEN
        PERFORM pg_notify('sms_notification', 
          jsonb_build_object(
            'recipient_id', v_user_id,
            'message', 'Your ' || v_service_name || ' appointment with ' || v_provider_name || ' has been rescheduled to ' || to_char(p_new_date, 'DD MMM, HH12:MI AM') || 
        ...
        PERFORM pg_notify('sms_notification', 
          jsonb_build_object(
            ...
            'message', 'Reschedule request from ' || v_client_name || ' for ' || v_service_name || '. They suggested ' || 
                     array_length(p_alternative_dates, 1) || ' alternative dates. Check your email or app to respond.',
            'data', v_notification_metadata
          )::text
        );
      END IF;
...
BEGIN
  ...
  BEGIN
    INSERT INTO chat_messages (
      ...
      v_chat_room_id,
      v_user_id, -- Message is from the client
      v_message_text,
      'delivered'
    );
  EXCEPTION WHEN OTHERS THEN
  ...
  -- Notify provider of the rescheduling request
  BEGIN
    -- Check provider notification preferences for appointments
    SELECT * INTO v_owner_preference 
    FROM notification_preferences 
    WHERE user_id = v_owner_id AND type = 'appointment';
    ...
    -- Trigger SMS notification for provider (more concise for SMS)
    IF v_owner_preference.sms THEN
      PERFORM pg_notify('sms_notification', 
        jsonb_build_object(
          'recipient_id', v_owner_id,
  ...
  -- Trigger system notification for rescheduling
  PERFORM pg_notify('appointment_rescheduled', v_result::text);
...
-- Create or replace respond_to_appointment_request function to use chat messaging
...
BEGIN
  ...
  LEFT JOIN 
    posts po ON s.post_id = po.id
  LEFT JOIN 
    profiles pr_owner ON p.owner_id = pr_owner.id
  LEFT JOIN 
    profiles pr_client ON p.user_id = pr_client.id
  WHERE 
    ap.id = p_appointment_id;
  ...
  IF v_source = 'appointment_purchase' THEN
    ...
    ELSIF p_action = 'select_client_date' THEN
      ...
      -- Update purchase dates
      UPDATE purchases
      SET start_date = p_alternative_time,
      ...
      HAVING COUNT(*) = 2 AND
             EXISTS (SELECT 1 FROM chat_participants WHERE chat_room_id = chat_participants.chat_room_id AND user_id = v_provider_id);
      ...
      v_message_text := 'Good news! I have selected one of your suggested dates for the ' || v_service_name || ' appointment.' ||
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      BEGIN
        ...
        -- Notify provider (owner) based on preferences
        IF v_provider_preference.in_app THEN
        INSERT INTO notifications (
          user_id,
          title,
          content,
        ...
        IF v_provider_preference.email THEN
          PERFORM pg_notify('email_notification', 
            jsonb_build_object(
              'recipient_id', v_provider_id,
              'type', 'appointment_payment_received',
              'data', v_notification_metadata
            )::text
          );
        ...
        IF v_provider_preference.push THEN
          PERFORM pg_notify('push_notification', 
            jsonb_build_object(
              'recipient_id', v_provider_id,
              'title', 'Payment Received',
        ...
        IF v_provider_preference.sms THEN
          PERFORM pg_notify('sms_notification', 
            jsonb_build_object(
              'recipient_id', v_provider_id,
              'message', 'Payment received for appointment on ' || to_char(v_appointment_date, 'DD MMM'),
...
-- Add a statement to check and remove the meeting_id column if needed in future migrations
-- This is just for reference, not to be executed directly, as we're still using NULL for it above
-- ALTER TABLE appointment_purchases DROP COLUMN IF EXISTS meeting_id; 
